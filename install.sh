...
sudo chmod +x /root/Xanomnsetup/Xanod /root/Xanomnsetup/Xano-cli
sudo mv /root/Xanomnsetup/Xanod /root/Xanomnsetup/Xano-cli /usr/local/bin
echo ""
echo ""
sleep 2
echo -e "${GREEN}Creating the ${RED}Configuration File Now${NC}"
sleep 2
sudo mkdir /root/.Xano
echo "rpcuser=user"`shuf -i 100000-10000000 -n 1` >> /root/.Xano/Xano.conf
echo "rpcpassword=pass"`shuf -i 100000-10000000 -n 1` >> /root/.Xano/Xano.conf
echo "rpcallowip=127.0.0.1" >> /root/.Xano/Xano.conf
echo "listen=1" >> /root/.Xano/Xano.conf
echo "server=1" >> /root/.Xano/Xano.conf
echo "daemon=1" >> /root/.Xano/Xano.conf
echo "maxconnections=250" >> /root/.Xano/Xano.conf
echo "masternode=1" >> /root/.Xano/Xano.conf
